import { PutRuleCommand, PutRuleCommandOutput } from '@aws-sdk/client-eventbridge';
...
export const putRule =
    ...
    async (input: PutRuleInput): Promise<PutRuleCommandOutput> => {
        ...
        const { name, eventPattern, scheduleExpression, state, eventBusName } = input;

        logger?.debug('putRule:start', { data: { name, eventBusName } });
